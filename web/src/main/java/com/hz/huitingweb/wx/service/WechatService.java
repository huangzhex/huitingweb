...
public class WechatService {
    ...
    public String processRequest(HttpServletRequest request) {
        ...
        if (WechatMessageUtil.MESSAGE_TEXT.equals(msgType)) {// 文本消息
            ...
            try {
                String tag=cosService.updateImageUrl(picUrl);
                responseMessage = createTextMessage(fromUserName,toUserName,"已接收图,生成key:"+tag);
            } catch (Exception e) {
                log.error(e.getMessage(),e);
                responseMessage = createTextMessage(fromUserName,toUserName,"保存图片失败");
            }
